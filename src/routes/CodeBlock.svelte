<script lang="ts">
  ...
  let copySuccess = $state<boolean | null>(null);
...
<div class="overflow-hidden border border-gray-300 dark:border-gray-700 rounded-md my-2">
  <div class="border-b border-gray-300 dark:border-gray-700 flex flex-row justify-between px-2 py-1">
    ...
    <div class="relative">
      <button onclick={copy} class={getClasses('px-2 py-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 text-black dark:text-white hover:bg-slate-200 dark:hover:bg-slate-500 active:bg-emerald-300 dark:active:bg-emerald-400', {'invisible': !!copySuccess})}>
        Copy
      </button>
      {#if copySuccess}
