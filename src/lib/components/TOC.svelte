...
<aside class={`w-[300px] overflow-hidden hidden mt-10 self-start md:!flex md:sticky md:top-14 flex-col justify-center items-center p-3
  ...
  <ul class="relative w-full max-h-[80vh] overflow-auto">
    {#each data as item}
      <li title={item.text}
  ...
  {#if highlightTop !== undefined && highlightBottom !== undefined}
    <!-- [TODO] mt-3 because there is p-3 in the parent container. Need more explicit way to handle this. -->
    <div class={'absolute left-0 bg-blue-400 w-1 mt-3'} style={`top: ${highlightTop}px; height:${highlightBottom - highlightTop}px`} />
  {/if}
</aside>
